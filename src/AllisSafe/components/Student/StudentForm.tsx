...
export const StudentForm = ({currentRol}) => {
              ...
              <div>
                <FormControl sx={{ m: 2, minWidth: 390 }} variant="outlined">
                  ...
                  <Select
                    ...
                    value={user.rol_id}
                    onChange={(e) => {
                      setUser({ ...user, rol_id: +e.target.value });
                    }}
                  >
   
                    {rols?.data?.filter((item)=>item?.id===currentRol).map((rol: any) => (
                      <MenuItem key={rol?.id} value={rol?.id }  selected={rol?.id === 3} >{rol?.name}</MenuItem>
                    ))}
                  </Select>
                </FormControl>
