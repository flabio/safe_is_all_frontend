...
export const QueryTodosCourseByIdSchool = async (id:number) => {
  const response: ApiResponses<DataResponse<ICourseSchool>> = await (await allIsSafeApi.get(`/course/school/${id}`)).data
  return (response).data
}
